...
# VA-1
render_env = gym.make("CartPole-v1", render_mode="human")  # only used for periodic viewing
def watch_episode(env, policy_net, device):
    ...
    for t in count():
        ...
        if done:
            ...
            if i_episode % 20 == 0:      # every 20 episodes, show a greedy run
                watch_episode(render_env, policy_net, device)            
